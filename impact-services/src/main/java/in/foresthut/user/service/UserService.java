...
import in.foresthut.impact.models.user.SignOutRequest;
import in.foresthut.impact.models.user.SignOutResponse;
import in.foresthut.impact.models.user.User;
import in.foresthut.user.entity.UserDao;
import in.foresthut.user.firebase.FirebaseInitializer;
...
public class UserService extends in.foresthut.impact.models.user.UserServiceGrpc.UserServiceImplBase {
    ...
    private final KeyValuePairHolder sessionRegistry;
    private GoogleIdToken.Payload userPayload;
    private FirebaseToken firebaseToken;

    public UserService(UserRepository userRepository, KeyValuePairHolder sessionRegistry) {
    ...
    public void signIn(in.foresthut.impact.models.user.SignInRequest request,
        ...
        if (!verifyFirebaseToken(request
                                         .getIdToken())) {
        ...
        // Put userId -> sessionToken in session registry
        //String userId = userPayload.getSubject();
        String userId = firebaseToken.getUid();
        sessionRegistry.add(userId, sessionToken);

        // If user does not exist in database
        if (userRepository.get(userId) == null) {
            var userDao = UserMapper.toUserDao(firebaseToken);
            if (userDao != null) {
    ...
    @Override
    public void signOut(SignOutRequest request, StreamObserver<SignOutResponse> responseObserver) {
        logger.info("Sign out createSiteRequest from user {}", request.getUserId());
        sessionRegistry.delete(request.getUserId());
        responseObserver.onNext(SignOutResponse.getDefaultInstance());
        responseObserver.onCompleted();
        logger.info("User {} logged out successfully.", request.getUserId());
    }
...
//        JsonFactory jsonFactory =
        ...
        try {
            ...
            if (idToken != null)
                userPayload = idToken.getPayload();
    ...
    private boolean verifyFirebaseToken(String idTokenString) {
        try {
            FirebaseInitializer.init();
            firebaseToken = FirebaseAuth.getInstance()
                                        .verifyIdToken(idTokenString);
            String uid = firebaseToken.getUid();
            logger.info("Verified Firebase user: '{}'", uid);
            return true;
