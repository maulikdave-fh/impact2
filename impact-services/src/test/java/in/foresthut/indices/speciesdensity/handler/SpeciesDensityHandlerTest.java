package in.foresthut.indices.speciesdensity.handler;
...
public class SpeciesDensityHandlerTest {
    ...
    void test() {
        ...
        SpeciesDensityRequest request = SpeciesDensityRequest.newBuilder()
                                                             .setSiteId(siteId)
                                                             .setForYear(2023)
                                                             .build();
        SpeciesDensityRequestHandler requestHandler = new SpeciesDensityRequestHandler(
                request, new OccurrenceRepositoryImpl(databaseConfig),
                new RestorationSiteRepositoryImpl(databaseConfig),
                new SpeciesDensityIndexRepositoryImpl(databaseConfig), null);

        var aoIBlocks = new RestorationSiteRepositoryImpl(databaseConfig).getAoI(siteId);
        SpeciesDensityRequestHandler.SpeciesDensityValueObj response  =requestHandler.calculateSpeciesDensity(aoIBlocks);

        System.out.println(response.speciesDensityIndexDao().aoiBlocks().get(0));
